...
class LunarUtil(cal: LocalDateTime) {
    ...
    private val chineseNumberCharacter = arrayOf("一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二")
    ...
    private fun init(cal: LocalDateTime) {
    ...
    private fun getChineseDayName(year: Int, month: Int, day: Int): String {
        val chineseTen = arrayOf("初", "十", "廿", "卅")
        val n = day % 10 - 1
        if (chineseFestival(year, month, day) != "")
            return chineseFestival(year, month, day)
        if (day > 30)
            return ""
        if (day == 1) {
            return if (month == 1)
                ...
                chineseNumberCharacter[month - 1] + "月"
        }
        if (day == 10)
            return "初十"
        if (day == 20)
        ...
        return if (day == 30)
            ...
            chineseTen[day / 10] + chineseNumberCharacter[n]
    }

    private fun chineseFestival(year: Int, month: Int, day: Int): String {
        when (month) {
            1 -> {
                when(day) {
                    1 -> return "春节"
    ...
    fun getChineseLunarDay(): String {
        return getChineseDayName(year, month, day)
    }
